...
class V1alpha3DeviceTaintSelector(BaseModel):
    ...
    """ # noqa: E501
    device: Optional[StrictStr] = Field(default=None, description="If device is set, only devices with that name are selected. This field corresponds to slice.spec.devices[].name.  Setting also driver and pool may be required to avoid ambiguity, but is not required.")
    device_class_name: Optional[StrictStr] = Field(default=None, description="If DeviceClassName is set, the selectors defined there must be satisfied by a device to be selected. This field corresponds to class.metadata.name.", alias="deviceClassName")
    driver: Optional[StrictStr] = Field(default=None, description="If driver is set, only devices from that driver are selected. This fields corresponds to slice.spec.driver.")
    ...
    selectors: Optional[List[V1alpha3DeviceSelector]] = Field(default=None, description="Selectors contains the same selection criteria as a ResourceClaim. Currently, CEL expressions are supported. All of these selectors must be satisfied.")
    __properties: ClassVar[List[str]] = ["device", "deviceClassName", "driver", "pool", "selectors"]
