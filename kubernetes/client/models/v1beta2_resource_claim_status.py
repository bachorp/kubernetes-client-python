...
class V1beta2ResourceClaimStatus(BaseModel):
    ...
    """ # noqa: E501
    allocation: Optional[V1beta2AllocationResult] = None
    devices: Optional[List[V1beta2AllocatedDeviceStatus]] = Field(default=None, description="Devices contains the status of each device allocated for this claim, as reported by the driver. This can include driver-specific information. Entries are owned by their respective drivers.")
    reserved_for: Optional[List[V1beta2ResourceClaimConsumerReference]] = Field(default=None, description="ReservedFor indicates which entities are currently allowed to use the claim. A Pod which references a ResourceClaim which is not reserved for that Pod will not be started. A claim that is in use or might be in use because it has been reserved must not get deallocated.  In a cluster with multiple scheduler instances, two pods might get scheduled concurrently by different schedulers. When they reference the same ResourceClaim which already has reached its maximum number of consumers, only one pod can be scheduled.  Both schedulers try to add their pod to the claim.status.reservedFor field, but only the update that reaches the API server first gets stored. The other one fails with an error and the scheduler which issued it knows that it must put the pod back into the queue, waiting for the ResourceClaim to become usable again.  There can be at most 256 such reservations. This may get increased in the future, but not reduced.", alias="reservedFor")
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "allocation": V1beta2AllocationResult.from_dict(obj["allocation"]) if obj.get("allocation") is not None else None,
            "devices": [V1beta2AllocatedDeviceStatus.from_dict(_item) for _item in obj["devices"]] if obj.get("devices") is not None else None,
            "reservedFor": [V1beta2ResourceClaimConsumerReference.from_dict(_item) for _item in obj["reservedFor"]] if obj.get("reservedFor") is not None else None
        })
        return _obj
